...
PERSIST_DIRECTORY = "chroma_db"  # Where to store ChromaDB data for policy
...
def get_vectorstore():
    try:
        # Check if we have a persisted ChromaDB
        if os.path.exists(PERSIST_DIRECTORY):
            ...
            elapsed = time.time() - start_time
            print(f"Loaded ChromaDB in {elapsed:.1f} seconds")
            return vectorstore, None
        
        # Load and process documents
        print("\nNo existing ChromaDB found. Creating new vectorstore...")
        start_time = time.time()
        
        print("\nStep 1/3: Loading PDF documents")
        documents = load_documents_from_folder(DOCUMENTS_FOLDER)
        if not documents:
            return None, "No valid documents found in the folder"
        ...
        print("\nStep 3/3: Creating vectorstore (this may take several minutes)")
        embeddings = HuggingFaceEmbeddings(model_name='all-MiniLM-L12-v2')
        ...
        for batch_num in range(total_batches):
            ...
            print(f"\rProcessing batch {batch_num + 1}/{total_batches}", end="")
        ...
        vectorstore.persist()
        elapsed = time.time() - start_time
        print(f"\nVectorstore creation complete in {elapsed:.1f} seconds")
        print(f"Total documents indexed: {len(split_documents)}")
...
def get_data_vectorstore():
    try:
        ...
        if os.path.exists(DATA_PERSIST_DIRECTORY):
        ...
        print("\nNo existing Data ChromaDB found. Creating new vectorstore...")
        start_time = time.time()
...
async def get_policy_answer(request: QuestionRequest):
    try:
        ...
        qa_chain = RetrievalQA.from_chain_type(
            ...
            return_source_documents=False  # Changed to False to prevent source docs from being processed
        )
...
async def get_data_answer(request: QuestionRequest):
    try:
        ...
        return {"answer": f"Error processing data request: {str(e)}"}
